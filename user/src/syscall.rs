...
pub fn sys_getcwd(buf: &mut [u8]) -> isize {
    syscall(SYSCALL_GETCWD, [buf.as_ptr() as usize, buf.len(), 0])
}
...
pub fn sys_unlink(path: &str, flags: u32) -> isize {
    syscall(SYSCALL_UNLINK, [path.as_ptr() as usize, flags as usize, 0])
}
